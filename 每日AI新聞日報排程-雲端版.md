...
>
> **v3 重大變更（多代理領域專家）**：改為 **Lead 協調者 + 5 位平行領域專家 + 1 個 Opus 品質審查** 的多代理流程。每個章節（主題）都由一位專屬領域專家撰稿，內容深度大幅提升（每篇文章約 1,000–1,600 漢字、結構化，並附深度範本供對照）。**模型分層**：搜集與撰寫用 **Claude Sonnet 5**（子代理，加速且省成本），只有協調、合併、跨章節綜整、品質審查等高品質關卡用 **Claude Opus**（Routine 主體）。
>
...
```jsonc
{
  ...
  "summaryMd": "150-250 字繁體中文重點摘要（Markdown；閘門下限 20 字，目標見「✍️ 內容深度規格」）",
...
# ✍️ 內容深度規格（**取代 v2 的「≥200 字」單薄要求**）

**每位領域專家動筆前，先用 Read 讀取深度範本 `templates/日報文章深度範本.md`**，以它的結構、段落編排與表格用法為準（只模仿深度、不照抄主題）。每篇撰稿務必達到以下深度（程式閘門下限雖仍是 200 字，但**品質審查會擋過短**）：
...
- **`contentMd`（全文內容）**：**約 1,000–1,600 漢字**，在 `## 📖 全文內容` 下切 **4–6 個 `###` 子章節**，敘事順序為：
  ...
  3. **具體數據**：用 **Markdown 表格**呈現數字、規格、名單、benchmark、融資條件等結構化資訊（每篇建議 2-3 個表格）。
  4. **影響 / 意義**：對產業、對台灣、對後續發展的影響。
  - 英文翻譯成通順繁體中文（專有名詞保留英文）；中文原文則保留。
- **`observationsMd`（觀察與啟發）**：2–3 點原創分析。**企業應用導入**須在地化到台灣（可點名玉山、中信、國泰世華等具體企業對照），並說明適用產業/角色、導入門檻、可借鏡之處。
- **學習筆記**：技術理論文章沿用上方「筆記 body 範本」10 段結構，至少一個 mermaid 圖或比較表、關鍵詞對照 5–10 條。

> 📎 **範本對照**：`templates/日報文章深度範本.md` 是一篇完整範例，示範「重點摘要 → 全文內容（含表格）→ 觀察與啟發」的深度與編排；其 Markdown 章節對應 JSON 的 `summaryMd` / `contentMd` / `observationsMd`。撰稿時比照其深度，**不要照抄其主題或數字**。
...
3. 撰稿（動筆前先 Read templates/日報文章深度範本.md 對照深度，再對齊「內容深度規格」）：每篇 summaryMd 150-250 字；contentMd 約 1,000-1,600 漢字、切 4-6 個 ### 子章節（背景→機制→具體數據含表格→影響）；observationsMd 2-3 點（企業應用導入要在地化台灣、填 industry）。
...
4. **內容深度規格**：每篇約 1,000-1,600 漢字、4-6 子章節、含表格；動筆前 Read `templates/日報文章深度範本.md` 對照；品質＞湊字數，全文太薄就換一篇。
